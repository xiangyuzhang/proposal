...
\date{May 2017} % The date you'll actually graduate -- must be
...
\firstreader{Maciej Ciesielski}
\secondreader{Sandip Kundu}
%\thirdreader{Bill Shepherd}
%\fourthreader{Mary Lamb}   % Optional
%\fifthreader{}            % Optional
%\sixthreader{}            % Optional
\departmentchair{Christopher V. Hollot} % Uses "Department Chair" as the title. To
...
This study is performed with two main goals in mind. The first goal is to implement the three current main stream gate-level circuit camouflage techniques as well as their performance. The second goal is to implement the Oracle-guided incremental de-camouflage algorithm. 

The four circuit camouflage algorithms are implemented by Python, and the Oracle-guided incremental de-camouflage algorithm is implemented by C++. During this study, I test the Oracle-guided de-camouflage tool (Solver, in short) performance by using it to de-obfuscate ISCAS-85 combinational benchmarks when camouflaged using the four camouflage algorithms. The results show that  Solver is able to efficiently de-obfuscate the ISCAS-85 benchmarks regardless of camouflaging style, and are able to do so 10.5x faster than the best existing approaches. And, based on Solver, this study also measurs the performance for each camouflage algorithms.
...
%The target of the attacker is to recover the function of the obfuscated circuit by finding a value of $P$ that induces his model to realize the same function as the oracle. Two functions are equivalent if they produce the same outputs for all possible inputs, but equivalence is usually checked symbolically instead of by exhaustively applying inputs. However, symbolic equivalence checking cannot be applied between a model and a black-box oracle, so SAT-based reverse engineering relies on an oracle-guided synthesis approach. Among all the possible functions of all camouflaged components, there must exist at least one values of $P$ to configure the model to be the same as the oracle. By using input-output examples from the oracle, the attacker can find the value of $P$ by ruling out values of until only a single function remains. Thus, the remaining function is equivalent to the oracle, and the value that induces the model to have this function is a solution to the obfuscation problem.
...
I evaluate my algorithm using a set of ISCAS-85 combinational benchmarks~\cite{hansen-99}. I also investigate the way to utilize both functionality and connection obfuscation. And all the results will be present in this chapter, This evaluation is based on the attacker model introduced in chapter 3.
